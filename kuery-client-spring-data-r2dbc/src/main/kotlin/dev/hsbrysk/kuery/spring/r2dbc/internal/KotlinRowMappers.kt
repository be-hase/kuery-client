 ...
 * as DataRetrievalFailureException, like the Spring mapper.
 */
internal class ValueClassPropertyRowMapper(
    ...
    override fun apply(readable: Readable): Any {
        val args = constructor.parameters.map { parameter ->
            val index = findColumnIndex(readable, checkNotNull(parameter.name))
            ...
            val raw = try {
                if (retrievalType != null) readable.get(index, retrievalType) else readable.get(index)
            } catch (@Suppress("SwallowedException") ex: IllegalArgumentException) {
                // The driver cannot convert to the requested type; retry raw and let
                // [convertValue] handle it through the ConversionService.
                readable.get(index)
            }
    ...
    private fun findColumnIndex(
        ...
        val row = readable as? Row
            ?: throw DataRetrievalFailureException("Cannot resolve column [$name]: row metadata is unavailable")
        val metadatas = row.metadata.columnMetadatas
        ...
        throw DataRetrievalFailureException("Unable to map column [$underscored] to constructor parameter [$name]")
    }

    // camelCase -> snake_case, mirroring JdbcUtils.convertPropertyNameToUnderscoreName.
    private fun underscoreName(name: String): String = buildString {

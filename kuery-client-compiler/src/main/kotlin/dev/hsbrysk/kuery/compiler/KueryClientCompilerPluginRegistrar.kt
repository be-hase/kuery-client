...
import dev.hsbrysk.kuery.compiler.KueryClientCompilerCommandLineProcessor.Companion.SQL_SYNTAX_CHECK_DIALECT_KEY
...
class KueryClientCompilerPluginRegistrar : CompilerPluginRegistrar() {
    ...
    override fun ExtensionStorage.registerExtensions(configuration: CompilerConfiguration) {
        ...
        val sqlSyntaxCheckDialect = configuration.get(SQL_SYNTAX_CHECK_DIALECT_KEY)
        // Configuring a dialect implies enabling the syntax check.
        val sqlSyntaxCheck = configuration.get(SQL_SYNTAX_CHECK_KEY, false) || sqlSyntaxCheckDialect != null
        val messageCollector = configuration.get(CommonConfigurationKeys.MESSAGE_COLLECTOR_KEY, MessageCollector.NONE)
        FirExtensionRegistrarAdapter.registerExtension(
            KueryClientFirExtensionRegistrar(autoTrimIndent, sqlSyntaxCheck, sqlSyntaxCheckDialect),
        )
